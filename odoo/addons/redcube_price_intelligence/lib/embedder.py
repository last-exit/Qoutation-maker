...
# The model is bind-mounted into the container at /opt/models (see docker-compose.yml),
# so it is not baked into the image and re-exporting it needs no rebuild.
ROOT = Path(os.environ.get("QE_MODEL_ROOT", "/opt/models"))
MODEL_NAME = "all-MiniLM-L6-v2"
ONNX_DIR = ROOT / MODEL_NAME
...
def get_embedder(force_backend=None):
    ...
    if backend == "onnx":
        if not onnx_available():
            raise RuntimeError(
                f"ONNX model not found at {ONNX_MODEL}. Run tools/export_onnx.py on the host; "
                f"docker-compose mounts ./models into the container at /opt/models."
            )
